...
class MinimizeGradientsBallDetector:
    ...
    def detect(self, image, w_H_p, playing_field_polygon, playground_mask):
        ...
        blurred = playing_field_polygon

        LAB = cv2.cvtColor(playing_field_polygon, cv2.COLOR_BGR2LAB)
        L = LAB[:,:,0]
        A = LAB[:,:,1]
        B = LAB[:,:,2]
        #cv2.imshow("L", L)
        ...
        HSV = cv2.cvtColor(playing_field_polygon, cv2.COLOR_BGR2HSV)
        H = HSV[:,:,0]
        S = HSV[:,:,1]
        V = HSV[:,:,2]
        #cv2.imshow("H", H)
        ...
        YCrCb = cv2.cvtColor(playing_field_polygon, cv2.COLOR_BGR2YCrCb)
        Y = YCrCb[:,:,0]
        Cr = YCrCb[:,:,1]
        Cv = YCrCb[:,:,2]
        #cv2.imshow("Y", Y)
        ...
        for keypoint in kpDark:
            ...
            this = sum_of_squared_gradients(playing_field_polygon[y - window_size:y + window_size + 1, x - window_size:x + window_size + 1])
        ...
        keypointsImg = playing_field_polygon.copy()
        cv2.drawKeypoints(playing_field_polygon, kpDark, keypointsImg, color=[0, 0, 255])
        ...
        for kp in best_kp:
            ...
            kp_y = kp.pt[1]
            search_radius = 18
            possible_ball = playing_field_polygon[kp_y - search_radius:kp_y + search_radius, kp_x - search_radius:kp_x + search_radius]
